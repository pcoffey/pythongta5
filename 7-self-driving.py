...
import keyboard
from keys import PressKey, W, A, S, D, right, straight, left, stop


def draw_lines(img, lines, color=[0, 255, 255], thickness=3):
    # if this fails, go with a default line
    try:

        # Find max y value for lane marker
        # cannot assume horizon in same place
        ys = []
        for i in lines:
            for ii in i:
                ys += [ii[1], ii[3]]
...
def process_img(image):
    ...
    try:
        ...
        cv2.line(original_image, (l1[0], l1[1]), (l1[2], l1[3]), [0,255,255], 30)
        cv2.line(original_image, (l2[0], l2[1]), (l2[2], l2[3]), [0,255,255], 30)
    except Exception as e:
        print(str(e))
        pass
    try:
